...
xp = yp = pTime = boxx = 0  # xp and yp - Previous coordinates of the pen (used for drawing lines). pTime - previous time for frames per second, boxx - variable to store the selected color box
tipIds = [4, 8, 12, 16, 20]  # List of landmark IDs for the tips of fingers.
# 4: Thumb tip
# 8: Index finger tip
# 12: Middle finger tip
# 16: Ring finger tip
# 20: Pinky finger tip
imgCanvas = np.zeros((480, 640, 3), np.uint8)  # a blank canvas to draw on
imgCanvasHistory = []

brushThickness = 5
top_height = 50  # height of the color section bar
Color = "Red"  # currently selected color
ColorList = {
    'Red': (0, 0, 255),
    'Green': (0, 255, 0),
    'Blue': (255, 0, 0),
    'Yellow': (0, 255, 255),
    'Black': (0, 0, 0),
}  # dictionary for mapping colors to BGR values
    ...
    while capture.isOpened():
        ...
        if len(lmList) != 0:
            ...
            if fingers[1] and not fingers[2] and math.hypot(x2 - x1, y2 - y1) > 50:
                ...
                # cv.circle(frame, (x1, y1), brushThickness, ColorList[Color], 2)
                xp, yp = x1, y1
